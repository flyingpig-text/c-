...
统一规范（本版本新增）：
    1) 全部物理量使用国际单位制（Pa、W、m、kg/m^3、W/(m·K)），
       打印时标注单位，并对关键量做数量级校验；
    2) 数据只从 D:\\46884\\Documents\\数学建模\\C题数据\\清洗后数据 读取：
       海水温度剖面、金属导热系数、海洋材料力学性能均来自该目录；
...
def build_material_table() -> pd.DataFrame:
    ...
    print("  * 价格/腐蚀速率不在数据包内，上表为工程默认【假设值，正式论文需替换引用】。")
    print()
