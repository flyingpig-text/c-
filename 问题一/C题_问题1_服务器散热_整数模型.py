# -*- coding: utf-8 -*-
"""
C 题 问题 1：单个圆柱集装箱最多可放多少台 1U 服务器

模型类型：退化的整数规划（最优解由直接取整获得，不使用任何求解器）

计算顺序（与《水下服务器热设计参数与算法交付清单》一致）：
    步骤 1：圆柱外表面积 A = πDL
    步骤 2：自然对流 Nu 关联式 -> 换热系数 h（水平圆柱 Churchill-Chu）
    步骤 3：Q_max = h A (T_max - T_inf)，N_theory = Q_max / q0
    步骤 4：空间上限 N_space = 内部容积 / 单台服务器体积
    步骤 5：N = floor(min(N_theory, N_space))，N_theory < 1 时输出 0

验收要求实现：
1. 全部采用国际单位制：m、m^2、m^3、W、W/(m^2·K)、K/W、Pa·s、kg/m^3；
   摄氏温度仅作工程读数，温差按 K 使用，每个物理量打印单位并做数量级校验。
2. 每一步打印中间结果：关键变量、中间公式、量纲、迭代残差，方便人工核对。
3. 数据只能从 C题数据 读取，优先信任 清洗后数据；找不到所需文件时报错，
   不使用编造的默认值。海水/空气物性采用文献关联式（非数据文件），并在输出中注明。
4. 全部中文注释；先运行简单基准算例（h=5、A=πDL、ΔT=60 K、N_space=800），
   通过后再运行完整自然对流模型。
5. 模型结束输出定量灵敏度分析：海水温度、限温、换热系数、服务器功率 ±5%/±10%，
...
def _find_data_dirs() -> tuple[Path, Path | None]:
    ...
    if data_dir is None:
        raise FileNotFoundError(
            "未找到 C题数据 文件夹。请确认脚本放在 D:\\46884\\Documents\\数学建模\\问题一 下。"
        )
